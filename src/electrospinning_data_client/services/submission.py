...
class SubmissionService:
    ...
    data). Unlike dataset reads, these endpoints require a personal access
    token created from the contributor's profile settings on the website.
    """
    ...
    def submit(self, payload: Dict[str, Any]) -> str:
        """POST a new experiment submission. Returns the raw server response text."""
    ...
    def update(self, experiment_id: int, payload: Dict[str, Any]) -> str:
        """PUT an update to an existing experiment. Returns the raw server response text."""
        headers = self._auth_headers()
        response = self._transport.request(
